...
class InterfaceEditarViagem:
    ...
    def show(self):
        ...
        print(f"Trip ID: {trip.trip_id}, Total KM: {trip.total_km}, Start Date: {trip.start_date}, End Date: {trip.end_date}")
        ...
        vehicle_selected = next((v for v in vehicle_options if str(trip.vehicle_id) in v), vehicle_options[0] if vehicle_options else "")
        ...
        if trip.description:
            self.description_entry.insert(0, trip.description)
        else:
        ...
        selected_driver_ids = {td.driver_id for td in trip_drivers} if trip_drivers else set()
        ...
        ListRoundedButton(button_frame, text="Salvar Alterações", command=lambda: self.save_trip(
    ...
    def save_trip(self, vehicle_combobox, additional_expenses_entry, total_km_entry, passenger_fare_entry, passenger_count_entry,
        ...
        try:
            ...
            trip = Trip(self.trip_id, vehicle_id, additional_expenses, total_km, passenger_fare, passenger_count, start_date_sql, end_date_sql, description)
            if self.trip_repo.update(trip):
                ...
                for driver_id, var in self.driver_vars.items():
                    if var.get():
                        trip_driver = TripDriver(self.trip_id, driver_id)
                        self.trip_driver_repo.add(trip_driver)

                messagebox.showinfo("Sucesso", f"Viagem atualizada com ID {self.trip_id} e motoristas associados!")
                self.back()
            else:
                messagebox.showerror("Erro", "Falha ao atualizar a viagem.")
