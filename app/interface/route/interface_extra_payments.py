...
class InterfaceRouteExtraPayments:
    ...
    def show(self):
        ...
        header_frame.pack(pady=(10, 5), padx=25, fill="x")

        tk.Label(
            header_frame,
            text=f"PAGAMENTOS EXTRAS: {self.route_name.upper()}",
        ...
        self.tree.column("Descricao", width=500, anchor="w", stretch=True)
    ...
    def load_extra_payments(self):
        ...
        all_payments = self.extra_payment_repo.get_all()
        ...
        for payment in route_payments:
            ...
            self.tree.insert("", "end", iid=str(payment.extra_payment_id), values=(
                ...
                payment.description.upper() if payment.description else ""
            ))

    def adicionar_pagamento_extra(self):
        InterfaceAddExtraPayment(self.parent, self.db_path, self.route_id).show()
    ...
    def excluir_pagamento_extra(self):
        ...
        if messagebox.askyesno("Confirmação", "Deseja excluir este pagamento extra?"):
            if self.extra_payment_repo.delete(int(selected[0])):
                self.load_extra_payments()
    ...
    def baixar_comprovante(self):
        ...
        payment = self.extra_payment_repo.get_by_id(int(selected[0]))
        ...
        if receipt_bytes.startswith(b"%PDF"):
            ext = ".pdf"
        elif receipt_bytes.startswith(b"\xff\xd8"):
            ext = ".jpg"
        elif receipt_bytes.startswith(b"\x89PNG"):
            ext = ".png"

        file_path = filedialog.asksaveasfilename(
            defaultextension=ext,
            filetypes=[("Arquivos", f"*{ext}")],
            initialfile=f"recibo_extra_{payment.extra_payment_id}{ext}"
        )
